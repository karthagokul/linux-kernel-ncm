...
static int ncm_setup(struct usb_function *f, const struct usb_ctrlrequest *ctrl)
{
	...
	printk(KERN_INFO "bRequestType: %i, bRequest: %i", ctrl->bRequestType, ctrl->bRequest);
	...
	case ((USB_DIR_OUT | USB_TYPE_VENDOR | USB_RECIP_DEVICE) << 8)
		| 0xF0:
	{
		DBG(cdev, "Terminal Mode command arrived");
		printk(KERN_INFO "Terminal Mode command arrived");
		value = 0;
		break;
	}
